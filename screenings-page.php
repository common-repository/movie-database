...
function movdb_screenings_page_main ()
{
	...
	<div class="wrap">
		...
		<ul class="subsubsub movdb-category">
			...
			<li class="at-home">
				<a class="<?php echo ($filter == 'at_home') ? 'current' : ''; ?>" href="admin.php?page=movdb_screenings_page&screening_filter=at_home">
					<?php _e('Seen at home', 'movdb'); ?>
					<span class="count">(<?php echo $screeningsTable->get_items_at_home(); ?>)</span>
				</a>
				|
			</li>
			<li class="in-cinemas">
				<a class="<?php echo ($filter == 'in_cinemas') ? 'current' : ''; ?>" href="admin.php?page=movdb_screenings_page&screening_filter=in_cinemas">
					<?php _e('Seen in cinemas', 'movdb'); ?>
					<span class="count">(<?php echo $screeningsTable->get_items_in_cinemas(); ?>)</span>
				</a>
			</li>
		</ul>
	...
	<form id="screening-edit" class="movdb-form" method="post" action="<?php echo admin_url('admin.php?page=movdb_screenings_page&action=' . $action . $id); ?>">
		<fieldset>
			...
			<ul>
				...
				<li class="<?php echo $labelType1; ?>">
					<label for="cinema"><?php _e('Cinema', 'movdb'); ?></label>
					<input type="text" id="cinema" name="cinema" value="<?php echo $args['cinema']; ?>" />
				</li>
				<li class="<?php echo $labelType1; ?>">
					...
					<select name="source">
						...
						if (!empty($args['movie']))
						{
							...
							echo '<option value="-1" ' . ($selected_source == '-1' ? 'selected="selected"' : '') . '>' . __('streaming', 'movdb') . '</option>';
							...
							echo '<option value="-1">' . __('streaming', 'movdb') . '</option>';
...
function movdb_screenings_page_handle_edit_form ($id = null)
{
	...
	$data = array (
		...
		'cinema' => stripslashes($_POST['cinema']),
