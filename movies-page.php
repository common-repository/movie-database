		...
		<fieldset>
			...
			<ul>
				...
				<li>
					...
					<input type="number" min="-100" max="32767" step="1" id="part" name="part" value="<?php echo $args['part']; ?>" />
				</li>
			</ul>
		</fieldset>
		<fieldset>
...
function movdb_ajax_movie_search()
{
	...
	$query = 'SELECT id, title, org_title, version, year, ' .
	         ...
	         'WHERE title like "%' . $search_query . '%" or org_title like "%' . $search_query . '%" or short_title like "%' . $search_query . '%" or series like "%' . $search_query . '%" ' .
