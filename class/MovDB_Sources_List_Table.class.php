...
class MovDB_Sources_List_Table extends WP_List_Table
{
	...
	**************************************************************************/
	function prepare_items ()
	{
		...
		*/
		...
		if (isset($_REQUEST['movies'])) {
			...
			$search = $_REQUEST['s'];
			$query .= 'where mov.title like "%' . $search . '%" or mov.org_title like "%' . $search . '%" or mov.short_title like "%' . $search . '%" or mov.series like "%' . $search . '%" ';
		}
		...
		foreach ($rawdata as $record)
		{
			...
			if ($this->get_current_filter() == 'type_uhd') {
				if ($record['type'] != 'uhd') {
					continue;
				}
			}
	...
	/**
	 * Gibt die Anzahl der Quellen vom Typ UHD-Blu-ray zurück.
	 */
	function get_items_type_uhd ()
	{
		return $this->items_type_uhd;
	}
