...
class MovDB_Statistics
{
	...
	function prepare_screenings ()
	{
		...
		$counter_screenings_at_home = 0;
		$counter_screenings_in_cinemas = 0;
		$total_cinemas = array();
		...
		foreach ($results as $result)
		{
			...
			if (empty($result['cinema']))
			{
				$counter_screenings_at_home++;
			}
			else
			{
				$counter_screenings_in_cinemas++;
				
				if (!isset($total_cinemas[$result['cinema']])) {
					$total_cinemas[$result['cinema']] = 0;
				}
				
				$total_cinemas[$result['cinema']]++;
			}
		}
		...
		arsort($total_cinemas);
		$total_cinemas_result = array();
		
		foreach ($total_cinemas as $name => $count) {
			$total_cinemas_result[] = $name . ' (' . $count . ')';
		}
		...
		$this->screenings_data = array(
			...
			__('Screenings at home', 'movdb') => $counter_screenings_at_home,
			__('in other cinemas', 'movdb') => $counter_screenings_in_cinemas,
			__('various cinemas', 'movdb') => '<abbr title="' . htmlspecialchars(implode(', ', $total_cinemas_result)) . '">' . count($total_cinemas_result) . '</abbr>',
			__('most frequent cinema', 'movdb') => count($total_cinemas_result) > 0 ? $total_cinemas_result[0] : '–',
		);
	}
	...
	function prepare_sources ()
	{
		...
		foreach ($results as $result)
		{
			if ($result['type'] == 'uhd') $counter_type_uhd++;
		...
		$this->sources_data = array(
			__('Sources', 'movdb') => sprintf(__('%1$s total (%2$s UHDs, %3$s Blu-rays, %4$s DVDs and %5$s as file)', 'movdb'), $sources_total, $counter_type_uhd, $counter_type_bluray, $counter_type_dvd, $counter_type_file),
