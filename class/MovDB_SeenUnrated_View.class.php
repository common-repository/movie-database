...
class MovDB_SeenUnrated_View extends MovDB_View
{
	...
	function get_item_content ($item)
	{
		...
		if (!empty($item['cinema'])) {
			$content[] = '<span class="cinema">' . __('at', 'movdb') . ' ' . $item['cinema'] . '</span>';
		}
	...
	function prepare_items ()
	{
		...
		$query = 'SELECT mov.id as movie, src.id as source, mov.title, mov.year, mov.rating, scr.guests, scr.cinema, ' .
