...
class MovDB_Widget extends WP_Widget
{
	...
	function content_latest_screenings ($instance)
	{
		...
		$query = 'SELECT `scr`.`id`, `scr`.`date`, `mov`.`title`, `mov`.`year`, `mov`.`rating`, `scr`.`guests`, `scr`.`cinema` ' .
		...
		foreach ($results as $result)
		{
			...
			<div class="movdb-widget-movie">
				...
				<?php endif; ?>
				<?php if (!empty($result['cinema'])): ?>
					<p class="cinema"><?php _e('at', 'movdb'); ?> <?php echo $result['cinema']; ?></p>
				<?php endif; ?>
